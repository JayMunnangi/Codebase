...
SELECT  @SPInfo = 
		...
		CASE @ProductName
				...
				WHEN 'SQL Server 2005' THEN	
											CASE @ProductVersion
												WHEN '9.00.5266' THEN 'You have already installed the latest cumulative update.'
												ELSE 'Install the latest Cumulative Update (CU) of SP4:  CU3, <https://support.microsoft.com/en-us/kb/2507769>'
											END
		END
...
BEGIN
	...
	IF (@ProductLevel = 'SP2') AND (@ProductVersion < '9.00.3356') 
	BEGIN
		SET @CumulativeUpdate = 'CU17'; SET @CumulativeUpdateKB = 'https://support.microsoft.com/en-us/kb/976952';
	END
	ELSE IF (@ProductLevel = 'SP3') AND (@ProductVersion < '9.00.4325')
	BEGIN
		SET @CumulativeUpdate = 'CU15'; SET @CumulativeUpdateKB = 'https://support.microsoft.com/en-us/kb/2507766';
	END
	ELSE IF (@ProductLevel = 'SP4') AND (@ProductVersion < '9.00.5266')
	BEGIN
		SET @CumulativeUpdate = 'CU3'; SET @CumulativeUpdateKB = 'https://support.microsoft.com/en-us/kb/2507769';
	END
...
BEGIN
	...
	END
	ELSE IF (@ProductLevel = 'SP1') AND (@ProductVersion < '12.0.4459.0')
	BEGIN
		SET @CumulativeUpdate = 'CU7'; SET @CumulativeUpdateKB = 'https://support.microsoft.com/en-us/kb/3162659';
	END
...
BEGIN
	...
	IF (@Build in ('9.00.1399','9.00.2047','9.00.3042','9.00.4035','9.00.5000'))
		SET @GDRorQFE = 1 
	ELSE IF (@Build in ('9.00.3233','9.00.3282','9.00.3310','9.00.3353','9.00.4262','9.00.4340','9.00.5292','9.00.5324'))
		SET @GDRorQFE = 2
	ELSE IF (@Build in ('9.00.3068','9.00.3073','9.00.3077','9.00.3080','9.00.4053','9.00.4060','9.00.5057','9.00.5069'))
		SET @GDRorQFE = 3
	ELSE 
		SET @GDRorQFE = 4

	SET @QFE = @QFE
		+ CHAR(13) + '9.00.3353 (SQL Server 2005 SP2 QFE) http://support.microsoft.com/kb/970896'
		+ CHAR(13) + '9.00.4340 (SQL Server 2005 SP3 QFE) http://support.microsoft.com/kb/2494112'
		+ CHAR(13) + '9.00.5324 (SQL Server 2005 SP4 QFE) http://support.microsoft.com/kb/2716427'

	SET @GDR = @GDR
		+ CHAR(13) + '9.00.3068 (SQL Server 2008 SP1 GDR) https://support.microsoft.com/en-us/kb/948109'
		+ CHAR(13) + '9.00.3073 (SQL Server 2008 SP2 GDR) http://support.microsoft.com/kb/954606'
		+ CHAR(13) + '9.00.3077 (SQL Server 2008 SP2 GDR) http://support.microsoft.com/kb/960089'
		+ CHAR(13) + '9.00.3080 (SQL Server 2008 SP3 GDR) http://support.microsoft.com/kb/970895'
		+ CHAR(13) + '9.00.4053 (SQL Server 2008 SP3 GDR) http://support.microsoft.com/kb/970892/'
		+ CHAR(13) + '9.00.4060 (SQL Server 2008 SP3 GDR) http://support.microsoft.com/kb/2494113'
		+ CHAR(13) + '9.00.5057 (SQL Server 2008 SP4 GDR) http://support.microsoft.com/kb/2494120'
		+ CHAR(13) + '9.00.5069 (SQL Server 2008 SP4 GDR) https://support.microsoft.com/en-us/kb/2716429'
END
