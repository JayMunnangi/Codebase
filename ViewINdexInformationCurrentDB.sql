...
--
-- 2012-04-05 Simplified execution by subdividing input queries
-- 2012-04-05 Fixed some collation issues;
-- 2012-04-06 Split in separate listings the unused indexes from rarely used indexes; Split in separate listings the real duplicates from possibly redundant indexes
-- 2012-06-06 Fixed issue with partition aligned indexes
-- 2012-10-31 Widened search for Redundant Indexes
-- 2012-12-17 Fixed several issues
-- 2013-01-17 Added several index related info
-- 2013-02-01 Fixed issue with Heap identification
-- 2013-02-26 Fixed issue with partition info; Removed alternate keys from search for Unused and Rarely used indexes
-- 2013-04-17 Added more information to duplicate and redundant indexes output, valuable when deciding which to delete, namely if duplicates are non-clustered
-- 2013-04-19 Fixed issue with potential duplicate index_ids in sys.dm_db_index_operational_stats relating to internal tables.
-- 2013-05-06 Changed data collection to minimize blocking potential on VLDBs.
-- 2013-05-20 Fixed issue with database names with special characters.
-- 2013-05-29 Fixed issue with large integers in aggregation.
-- 2013-06-20 Added step to avoid entering in loop that generates dump in SQL 2005.
-- 2013-11-10 Added index checks.
-- 2014-02-24 Added info to Unused_IX section.
-- 2014-06-04 Refined search for duplicate and redundant indexes.
-- 2014-11-12 Added SQL 2014 Hash indexes support; changed scan mode to LIMITED; added search for hard coded references to indexes.
...
BEGIN
	...
	SELECT ps.database_id, 
		...
		ps.record_count,
		ps.forwarded_record_count -- for heaps
...
SELECT 'All_Heaps_Info' AS [Category], [database_id], [database_name], [object_id], [schema_name], [table_name], [index_id], [type_desc] AS index_type,
	[partition_number], [page_count], [size_MB], record_count, forwarded_record_count, forwarded_fetch_count,
