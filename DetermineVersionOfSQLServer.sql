		...
		CASE @ProductName
			    WHEN 'SQL Server 2016' THEN 
											CASE @ProductVersion
												...
												ELSE 'Install the latest Cumulative Update (CU) of SP1:  CU3, <https://support.microsoft.com/en-us/kb/4019916>'
											END
				WHEN 'SQL Server 2014' THEN 
											CASE @ProductVersion
												WHEN '12.0.5546.0' THEN 'You have already installed the latest cumulative update.'
												ELSE 'Install the latest Cumulative Update (CU) of SP2:  CU5, <https://support.microsoft.com/en-us/kb/4013098>'
											END
				WHEN 'SQL Server 2012' THEN
											CASE @ProductVersion
												WHEN '11.0.6598.0' THEN 'You have already installed the latest cumulative update.'
												ELSE 'Install the latest Cumulative Update (CU) of SP3:  CU9, <https://support.microsoft.com/en-us/kb/4016762>'
											END
...
BEGIN
	...
	END
	ELSE IF (@ProductLevel = 'SP3') AND (@ProductVersion < '11.0.6598.0')
	BEGIN
		SET @CumulativeUpdate = 'CU9'; SET @CumulativeUpdateKB = 'https://support.microsoft.com/en-us/kb/4016762';
	END
	SELECT  @OtherProduct = 
			CASE 
				WHEN @ProductLevel = 'RTM' THEN ' If you want to upgrade to higher edition, please intall service pack.'
...
BEGIN
	...
	END
	ELSE IF (@ProductLevel = 'SP1') AND (@ProductVersion < '12.0.4502.0')
	BEGIN
		SET @CumulativeUpdate = 'CU11'; SET @CumulativeUpdateKB = 'https://support.microsoft.com/en-us/kb/4010392';
	END
	ELSE IF (@ProductLevel = 'SP2') AND (@ProductVersion < '12.0.5546.0')
	BEGIN
		SET @CumulativeUpdate = 'CU5'; SET @CumulativeUpdateKB = 'https://support.microsoft.com/en-us/kb/4013098';
	END
...
BEGIN
	...
	--2016
	IF (@ProductLevel = 'RTM') AND (@ProductVersion < '13.0.2204.0')
	BEGIN
		SET @CumulativeUpdate = 'CU6'; SET @CumulativeUpdateKB = 'https://support.microsoft.com/en-us/kb/4019914';
	END
	ELSE IF (@ProductLevel = 'SP1') AND (@ProductVersion < '13.0.4435.0')
	BEGIN
		SET @CumulativeUpdate = 'CU3'; SET @CumulativeUpdateKB = 'https://support.microsoft.com/en-us/kb/4019916';
	END			
END
